...
uses
  ...
  Vcl.ExtCtrls, interfaceMyFrame,  ParentUnit, Registry;
...
begin
  ...
  begin if (Keyfont1.Size>=15) then keyfont1.Size:=15; end else
  if Keyfont1.Size>=10 then keyfont1.Size:=10;
...
begin
  if FontDialog2.Execute then KeyFont2:=FontDialog2.Font;
  if (BackForm.activeForm.ClassName='TKeyboardFormLarge') then
  begin if (Keyfont2.Size>=15) then keyfont2.Size:=15; end else
  if Keyfont2.Size>=10 then keyfont2.Size:=10;
end;
...
begin
   ...
   t.Style:=FS;
   result:=t;  reg.CloseKey;
   reg.OpenKey(reg.FileName, false);
   //
end;
begin
