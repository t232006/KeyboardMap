...
uses
  ...
  Vcl.Menus, KeyboardUnit, saver, Key, Utilites;
type
  TForm1 = class(TForm)
    Memo1: TMemo;
    TrayIcon: TTrayIcon;
    ...
    procedure Key18MouseDown(Sender: TObject; Button: TMouseButton;
...
procedure TForm1.Key18MouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
  var
    state: WindowPlacement; ch:char;
begin
    if GetWinHandle<>0 then
      hwin:= GetWinHandle;//strtoint(label2.Caption) ;
    //if hwin<>self.Handle then
    state.length:=sizeof(windowplacement);
    GetWindowPlacement(hwin, state);
    ShowWindow(hwin, state.showCmd);
    BringWindowToTop(hwin);
    ch:=(sender as TKey).MiddleText[length((sender as TKey).MiddleText)];
    SendKeypress(ord(ch),false);
end;
